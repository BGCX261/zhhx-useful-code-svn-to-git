...
void decode_one_b8block (int decoder, int mbmode, int b8block, int b8mode, int b8ref)
{
  ...
  else
  {
    ...
    {
      for(i=i0;i<i1;i++)//i0,j0Îª8*8
      for(j=j0;j<j1;j++)
      {
        resY_tmp[j][i]=0;
      }
      for (by=by0; by<by1; by++)//bx0,by0Îª4¸ö4*4 blockÓÃ
...
byte Get_Reference_Pixel(byte **imY, int y_pos, int x_pos)
{
  ...
  if (dx == 0 && dy == 0) { /* fullpel position */
    ...
    if (dy == 0) {
      ...
      result = max(0, min(255, (result+16)/32));//°ëÏñËØ²åÖµ,dx==2µÄÇé¿ö

      if (dx == 1) {
        result = (result + imY[pres_y][max(0,min(maxold_x,x_pos))])/2;//1/4ÏñËØ²åÖµ
      }
      else if (dx == 3) {
        result = (result + imY[pres_y][max(0,min(maxold_x,x_pos+1))])/2;//1/4ÏñËØ²åÖµ
      }
    }
    else if (dx == 0) {
      ...
      result = max(0, min(255, (result+16)/32));//°ëÏñËØ²åÖµ,dy==2µÄÇé¿ö

      if (dy == 1) {
        result = (result + imY[max(0,min(maxold_y,y_pos))][pres_x])/2;//1/4ÏñËØ²åÖµ
      }
      else if (dy == 3) {
        result = (result + imY[max(0,min(maxold_y,y_pos+1))][pres_x])/2;//1/4ÏñËØ²åÖµ
      }
    }
    else if (dx == 2) {

      for(y=-2;y<4;y++) {//ÏÈ¼ÆËã6¸ö°ëÏñËØ
        result = 0;
        pres_y = max(0,min(maxold_y,y_pos+y));
        for(x=-2;x<4;x++) {
          pres_x = max(0,min(maxold_x,x_pos+x));
      ...
      result = 0;
      for(y=-2;y<4;y++) {//dy==2Ê±µÄ°ëÏñËØ²åÖµ
      ...
      if (dy == 1) {
        result = (result + max(0, min(255, (tmp_res[2]+16)/32)))/2;//1/4ÏñËØ²åÖµ
      }
      else if (dy == 3) {
        result = (result + max(0, min(255, (tmp_res[3]+16)/32)))/2;//1/4ÏñËØ²åÖµ
      }
    }
    else if (dy == 2) {
      ...
      result1 = max(0, min(255, (result+16)/32));//¼ÆËãÒ»¸ö°ëÏñËØ²åÖµ
      ...
      result2 = max(0, min(255, (result+16)/32));//¼ÆËãÁíÒ»¸ö°ëÏñËØ²åÖµ
      result = (result1+result2)/2;//1/4ÏñËØ²åÖµ
    }
  }
...
void compute_residue_b8block (int b8block, int i16mode) // if not INTRA16x16 it has to be -1
{
  ...
  if (i16mode>=0)
  {
    ...
    for (j=j0; j<j1; j++)
    {
      decs->resY[j][i] = enc_picture->imgY[img->pix_y+j][img->pix_x+i] - img->mprr_2[i16mode][j][i];//Ö¡ÄÚÔ¤²â²Ð²î
    }
  }
  else
  {
    ...
    for (j=j0; j<j1; j++)
    {
      decs->resY[j][i] = enc_picture->imgY[img->pix_y+j][img->pix_x+i] - img->mpr[i][j];//Ö¡¼äÔ¤²â²Ð²î
    }
  }
}
