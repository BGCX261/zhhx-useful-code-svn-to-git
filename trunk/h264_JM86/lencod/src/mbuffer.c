...
StorablePicture **listX[6];//0,1为帧的list0, 1; index 2,3为当前场为顶场list 0,1; index 4,5为当前场为底场的 list 0,1
...
static void gen_pic_list_from_frame_list(PictureStructure currStrcture, FrameStore **fs_list, int list_idx, StorablePicture **list, int *list_size, int long_term)
{
  ...
  if (currStrcture == TOP_FIELD)//顶场底场交替
  {
  ...
  if (currStrcture == BOTTOM_FIELD)//底场顶场交替
  {
    while ((top_idx<list_idx)||(bot_idx<list_idx))
    {
      for ( ; bot_idx<list_idx; bot_idx++)
...
void init_lists(int currSliceType, PictureStructure currPicStructure)
{
  ...
  //计算参考帧的frame num及pic num,用于参考list的初始化
  if (currPicStructure == FRAME)  //当前图像为帧
  {
    for (i=0; i<dpb.ref_frames_in_buffer; i++)
    {
      if (dpb.fs_ref[i]->is_used==3)
      {
        if ((dpb.fs_ref[i]->frame->used_for_reference)&&(!dpb.fs_ref[i]->frame->is_long_term))//短期参考
        {
          if( dpb.fs_ref[i]->frame_num > img->frame_num )//先计算变量frame_num_wrap
          {
          ...
          dpb.fs_ref[i]->frame->pic_num = dpb.fs_ref[i]->frame_num_wrap;//参考帧排序用的pic_num
          dpb.fs_ref[i]->frame->order_num=list0idx;//短期参考帧在list0的开始offset， ##### list0idx为什么没有变化，所有参考帧的order_num都为0
        }
      }
    }
  }
  else
  {
    ...
    for (i=0; i<dpb.ref_frames_in_buffer; i++)
    {
      if (dpb.fs_ref[i]->is_reference)
      {
        ...
        if (dpb.fs_ref[i]->is_reference & 1)//参考帧有顶场
        {
          dpb.fs_ref[i]->top_field->pic_num = (2 * dpb.fs_ref[i]->frame_num_wrap) + add_top;//相同奇偶性，就+1为奇数
        }
        if (dpb.fs_ref[i]->is_reference & 2)//参考帧有底场
        {
  ...
  if ((currSliceType == P_SLICE)||(currSliceType == SP_SLICE))//p slice时初始化参考列表0
  {
    
    if (currPicStructure == FRAME)  //当前图像是帧
    {
      for (i=0; i<dpb.ref_frames_in_buffer; i++)
      {
        if (dpb.fs_ref[i]->is_used==3)
      ...
      qsort((void *)listX[0], list0idx, sizeof(StorablePicture*), compare_pic_by_pic_num_desc);//根据PicNum排序
      listXsize[0] = list0idx;//参考列表0的长度
//      printf("listX[0] (PicNum): "); for (i=0; i<list0idx; i++){printf ("%d  ", listX[0][i]->pic_num);} printf("\n");
      ...
      for (i=0; i<dpb.ltref_frames_in_buffer; i++)
      {
        if (dpb.fs_ltref[i]->is_used==3)
        {
          if (dpb.fs_ltref[i]->frame->is_long_term)
          {
            ...
            dpb.fs_ltref[i]->frame->order_num=list0idx;//长期参考帧在list0的开始offset
    ...
    else
    {
	 //当前图像是场
      ...
      for (i=0; i<dpb.ref_frames_in_buffer; i++)//首先短期参考帧排序
      {
        if (dpb.fs_ref[i]->is_reference)
        {
...
//      printf("fs_list0 (FrameNum): "); for (i=0; i<list0idx; i++){printf ("%d  ", fs_list0[i]->frame_num_wrap);} printf("\n");
      ...
      listXsize[0] = 0;
      gen_pic_list_from_frame_list(currPicStructure, fs_list0, list0idx, listX[0], &listXsize[0], 0);//产生短期参考场列表

//      printf("listX[0] (PicNum): "); for (i=0; i<listXsize[0]; i++){printf ("%d  ", listX[0][i]->pic_num);} printf("\n");

      // long term handling
      for (i=0; i<dpb.ltref_frames_in_buffer; i++)//长期参考帧排序
      {
      ...
      gen_pic_list_from_frame_list(currPicStructure, fs_listlt, listltidx, listX[0], &listXsize[0], 1);//产生长期参考场列表
