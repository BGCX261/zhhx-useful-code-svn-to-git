...
*/
  ...
  srcframe = AllocSourceframe (img->width, img->height);//分配frame,top field,bottom field的y,u,v内存
...
static void init_frame ()
{
  ...
  for(i=0;i< ((img->width/MB_BLOCK_SIZE)*(img->height/MB_BLOCK_SIZE));i++)
    img->mb_data[i].slice_nr=-1;//初始化时所有宏块的slice no都初始化为-1
...
void UnifiedOneForthPix (StorablePicture *s)
{
  ...
  for (j = -IMG_PAD_SIZE; j < s->size_y + IMG_PAD_SIZE; j++)
  {
    for (i = -IMG_PAD_SIZE; i < s->size_x + IMG_PAD_SIZE; i++)
    {
	  ...
	  //ONE_FOURTH_TAP 2维数组ONE_FOURTH_TAP[0][1]ONE_FOURTH_TAP[1][1]ONE_FOURTH_TAP[2][1]没有用
	  //ONE_FOURTH_TAP[0][0]=20 ONE_FOURTH_TAP[1][0]=-5 ONE_FOURTH_TAP[2][0]=1对应标准求半像素公式(E-5F+20G+20H-5I+J)/32的系数
	  //E F GbH I J其中b是半像素位置，其他大写字母是整数像素位置，依据距离远近加权，得到预测系数，当然我认为这些tap系数可以修改
      is =
            ...
            img4Y_tmp[j + IMG_PAD_SIZE][(i + IMG_PAD_SIZE) * 2] = imgY[jj][max (0, min (s->size_x - 1, i))] * 1024;    //// 整数像素 1/1 pix pos
            img4Y_tmp[j + IMG_PAD_SIZE][(i + IMG_PAD_SIZE) * 2 + 1] = is * 32;  //这里本来该除32的，却乘了32,定标法而已　看后面除以1024就明白 1/2 pix pos
    }
  }
...
*/
    ...
    GenerateFullPelRepresentation (out4Y, ref11, s->size_x, s->size_y);//把out4Y复制给ref11
...
static void ReadOneFrame (int FrameNoInFile, int HeaderSize, int xs, int ys, Sourceframe *sf)
{
  ...
  // top and bottom field (sf->?t and sf->?b)
	//分出top field,bottom field的YUV数据
