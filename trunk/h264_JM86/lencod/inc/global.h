...
int    **img4Y_tmp;          //!< for quarter pel interpolation 存放整数及水平1/2像素内插的临时变量
...
byte   **imgY_org_frm;//Y原始数据,这个数组要以imgY_org_frm[height][width]引用,如在代码中以[y][x]下标
byte  ***imgUV_org_frm;//UV原始数据,这个数组要以imgY_org_frm[U/V][height][width]引用
...
typedef struct
{
  ...
  int mprr[9][16][16];         //!< all 9 prediction modes? // enlarged from 4 to 16 for ABT (is that neccessary?)	//++ 存放亮度 intra4*4 九种预测模式的预测值

  int mprr_2[5][16][16];       //!< all 4 new intra prediction modes	//++ 存放亮度 intra16*16 四种预测模式的预测值，mprr_2[5]？？？
  int mprr_c[2][4][8][8];      //!< new chroma 8x8 intra prediction modes	//++ 存放色度四种预测模式的预测值
