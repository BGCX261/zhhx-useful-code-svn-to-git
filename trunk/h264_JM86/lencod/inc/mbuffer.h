...
typedef struct storable_picture
{
  ...
  byte **     imgY;          //!< Y picture component	//++ 当前图像已编码像素亮度重构值
  ...
  byte ***    imgUV;         //!< U and V picture components	//++ 当前图像已编码像素色度重构值

  byte *      mb_field;      //!< field macroblock indicator

  int  ***    ref_idx;       //!< reference picture   [list][subblock_x][subblock_y]
                             //   [list][mb_nr][subblock_x][subblock_y]//参考帧索引
...
typedef struct decoded_picture_buffer
{
  ...
  unsigned      size;
  unsigned      used_size;
  unsigned      ref_frames_in_buffer;//非长期参考帧数,但是并不是这里的每一帧都用于参考了,有时也没有参考到
  unsigned      ltref_frames_in_buffer;//long time长期参考帧数
  int           last_output_poc;
